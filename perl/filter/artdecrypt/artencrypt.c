...
#define PARAMNUM                4
int decrypt_main(int argc, char* argv[], int un)
{
    ...
    JESDEBUG("[%s]dncrypt_main infile outfile argc=%d un=%d", strFuncName, argc, un);
    ...
    if ( 1 == g_flag_pipe_dec ) {
		...
		pInFileName=argv[un+1];
		pOutFileName=argv[un+2];
    ...
    if(optDecrypt && (mode == ecb || mode == cbc))
 	...
 	JESTRACE("Begin to call CryptData() for decrypt");
    CryptData();   
}

int encrypt_main(int argc, char *argv[], int nu)
{
    ...
    JESDEBUG("[%s]encrypt_main argc=%d un=%d", strFuncName, argc, nu);
	...
	pInFileName=argv[nu+1];
	pOutFileName=argv[nu+2];
    JESDEBUG("[%s]key=%s", strFuncName, g_key);
    if(strcmp(pInFileName, pOutFileName) == 0)
    {
        JESTRACE("source and destination are identical!");
        exit(-1);
    }
...
void print_usage() {
 	JESTRACE("Usage: artencrypt [-d] [-f] [-h] [infile] [outfile]");
	JESERR("usage error!");
}
...
int main(int argc, char *argv[]) {
    ...
    while ((option = getopt(argc, argv,"dfk:h")) != -1) {
        switch (option) {
             case 'd' : flag_dec = 1;
                 break;
             case 'f' : g_flag_pipe_dec = 0;
                 break;
             case 'k' : strncpy(g_key, optarg, ART_MAX_KEY_LEN-1);
                        un++;
                 break;
    ...
    if (flag_dec == 0) {
		/* encrypt */
        return encrypt_main(argc, argv, un);
    } else {
		/* decrypt */
        return decrypt_main(argc, argv, un);
	}
	return 0;
}
